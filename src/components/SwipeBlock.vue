<template>
  <div 
    v-on:touchmove="debouncedTouchMove" 
    v-on:touchstart="touchStart" 
    v-on:touchend="touchEnd">
    <div class='braille-grid'>
      <b-row class='braille-row my-4' v-bind:key="index" v-for="(row, index) in brailleSentence[wordIndex][letterIndex]">
          <b-col :data-row="index" :data-col="0" class='border border-dark p-4 d-flex justify-content-center align-items-center'>
              <div  :data-row="index" :data-col="0" :class="row[0] != 0 ? 'dot' : '' "></div>
          </b-col>
          <b-col style='background-color: white;'></b-col>
          <b-col :data-row="index" :data-col="1"  class='border border-dark p-4 d-flex justify-content-center align-items-center'>
              <div :data-row="index" :data-col="1" :class="row[1] != 0 ? 'dot' : '' "></div>
          </b-col>
      </b-row>

    </div>
    <b-row class="my-4 text-center">
      <b-col>
        <b-btn @click="changeIndex(-1)" :disabled="wordIndex == 0">Previous</b-btn>
      </b-col>
      <b-col>
        <h1> {{ currentLetter }} </h1>
      </b-col>
      <b-col>
        <b-btn @click="changeIndex(1)" :disabled="letterIndex == (currentWord.length - 1) && wordIndex == (sentence.length -1)">Next</b-btn>
      </b-col>
    </b-row>
    <b-row class="my-4 text-center">
      <b-col>
        <b-btn-group>
          <b-btn variant="primary" @click="swipeToggle" :pressed.sync="swipeToggle">Swipe to Change</b-btn>
          <b-btn variant="primary" @click="tiltToggle" :pressed.sync="tiltToggle">Tilt to Change</b-btn>
        </b-btn-group>
      </b-col>
    </b-row>
  </div>
</template>

<script>
import _ from "lodash";
import BrailleService from '../services/BrailleConverter';

export default {
  name: "swipe-block",
  props: {
    word: {
      type: String,
      required: true,
      default: ''
    },
    sentence: {
      type: Array,
    }
  },
  data() {
    return {
      swipeToggle: true,
      tiltToggle: true,
      wordIndex: 0,
      letterIndex: 0,
      matrix: null,
      text: null, 
      swipedDirection: null,
      gamma: 0,
      beta: 0,
      pageX: null,
      pageY: null,
      xValue: null,
      yValue: null
    };
  },
  computed: {
    currentWord: function() {
      return this.sentence[this.wordIndex]
    },
    brailleSentence: function() {
      let resArray = []
      // Convert the 
      this.sentence.forEach((w) => {
        // Get the braille word
        resArray.push(BrailleService.wordToBraile(w));
      });
      return resArray;
    },
    // brailleArray: function() {
    //   return BrailleService.wordToBraile(this.word);
    // },
    currentLetter: function() {
      // Get the letter at the current word 
      return this.currentWord[this.letterIndex]
    }
  },
  created() {
    this.debouncedTouchMove = _.debounce(this.getCurrentTouch, 100, {
      leading: true, trailing: false
    });
    if ( window.DeviceMotionEvent ) {
      window.ondeviceorientation = _.throttle(this.rotateDevice, 1000)
    }
  },
  methods: {
    toggleTilt() {
      this.tiltToggle = !this.tiltToggle;
    },
    toggleSwipe() {
      this.swipeToggle = !this.swipeToggle;
    },
    rotateDevice({ beta, gamma }) {
      this.gamma += (gamma - this.gamma) / 5;
      // let changeX = (beta - this.beta) / 5;
      if (gamma >= 25 && this.tiltToggle) {
        this.changeIndex(1);
      } else if (gamma <= -25 && this.tiltToggle) {
        this.changeIndex(-1);
      }
    },
    /** 
     * Change the letter we are looking at by pressing a button
     */
    changeIndex(changeVal) {
      // Check the letter to see if it's the end
      let changeIndex = this.letterIndex + changeVal;
      let isValidLetterIndex = (changeIndex >= 0 && changeIndex <= (this.currentWord.length - 1))
      // Can we go left or right
      if ( isValidLetterIndex ) {
        var snd = new Audio("data:audio/wav;base64,");  
        snd.play();
        this.letterIndex = changeIndex;
        
      } 
      // Now at the beginning of a new word?
      else {
        // Check if our letter index overruns our word
        if (changeIndex < 0 && this.wordIndex > 0 ) {
          this.wordIndex -= 1
        } else if (changeIndex > (this.currentWord.length - 1) && this.wordIndex < (this.sentence.length - 1)) {
          this.wordIndex += 1
        }
        // Reset the letter index to the first letter
        this.letterIndex = 0;

      }
    },
    /**
     * Get the user's position on the page
     */
    getCurrentTouch({ touches }) {
      try {
        this.pageX = touches[0].pageX;
        this.pageY = touches[0].pageY;

        let el = document.elementFromPoint(this.pageX, this.pageY);
        var { row, col } = el.dataset;

        row = parseInt(row);
        col = parseInt(col);

        // Check the word
        if (this.brailleSentence[this.wordIndex][this.letterIndex][row][col] == 1) {
          console.log("HIT")
          window.navigator.vibrate(200);
        }
      } catch (e) { console.log(e)}
    },
    touchStart({ touches }) {
      this.xValue = touches[0].clientX;
      this.yValue = touches[0].clientY;
    },
    /**
     * At the end, identify the swipe direction
     */
    touchEnd({ changedTouches }) {
      if (!this.xValue || !this.yValue) {
        return;
      }

      const touches = changedTouches;
      let currentX = touches[0].clientX;
      let currentY = touches[0].clientY;

      let diffX = this.xValue - currentX;
      let diffY = this.yValue - currentY;

      if (Math.abs(diffX) > Math.abs(diffY)) {
        // Sliding horizontally
        if (diffX > 20 & this.swipeToggle) {
          this.text = "Left";
          this.changeIndex(-1)
        } else if (diffX < -20 & this.swipeToggle) {
          this.text = "Right";
          this.changeIndex(1)
        }
      } else {
        if (diffY > 0) {
          this.text = "Up";
        } else {
          this.text = "Down";
        }
      }

      this.xValue = null;
      this.yValue = null;
    }
  }
};
</script>

<style lang="scss">
.braille-grid {
  border: 2px solid black;

  .braille-row {
    min-height: 100px;
    background-color: #ececec;
  }
}

.dot {
  width: 50px;
  height: 50px;
  background-color: black;
  border-radius: 50px;
}
</style>
